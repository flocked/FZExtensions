...
public struct Path: Pathish {
    ...
    public init?<S: StringProtocol>(_ description: S) {
        ...
        #endif
            let path = join_(prefix: "/", pathComponents: pathComponents)
        ...
        case "~":
            ...
            let path = join_(prefix: tilded, pathComponents: pathComponents)
            url = URL(fileURLWithPath: path)
...
public extension Pathish {
    ...
    var parent: Path {
        ...
        guard index != path.indices.startIndex else { return Path(string: "/") }
        let substr = path[path.indices.startIndex..<index]
    ...
    */
    ...
    func join<S>(_ addendum: S) -> Path where S: StringProtocol {
        return Path(string: join_(prefix: path, appending: addendum))
    }
    ...
    func relative<P: Pathish>(to base: P) -> String {
        ...
        let baseComps = (base.path as NSString).pathComponents
    ...
    func basename(dropExtension: Bool = false) -> String {
        var lastPathComponent: Substring {
            ...
            let index = path.index(after: slash)
            return path[index...]
        }
        var go: Substring {
            if !dropExtension {
    ...
    func readlink() throws -> Path {
        do {
            let rv = try FileManager.default.destinationOfSymbolicLink(atPath: path)
    ...
    func realpath() throws -> Path {
        guard let rv = _realpath(path, nil) else { throw CocoaError.error(.fileNoSuchFile) }
        defer { free(rv) }
    ...
    static func <(lhs: Self, rhs: Self) -> Bool {
        return lhs.path.compare(rhs.path, locale: .current) == .orderedAscending
    }
}
...
public struct DynamicPath: Pathish {
    ...
    public subscript(dynamicMember addendum: String) -> DynamicPath {
        ...
        return DynamicPath(string: join_(prefix: path, appending: addendum))
    }
}
