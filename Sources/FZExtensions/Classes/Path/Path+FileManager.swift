...
public extension Pathish {
    ...
    #endif
        try FileManager.default.copyItem(atPath: path, toPath: to.path)
        return Path(to)
    }
    ...
    */
    ...
    func symlink<P: Pathish>(as: P) throws -> Path {
        try FileManager.default.createSymbolicLink(atPath: `as`.path, withDestinationPath: path)
        return Path(`as`)
    }
    ...
    /**
     ...
     - Note: If `self` does not exist, is **not** an error.
     */
    var symlinkDestionation: Path? {
        if (isSymlink) {
            guard let symPath = try? FileManager.default.destinationOfSymbolicLink(atPath: path) else { return nil }
            return Path(symPath)
        }
        return nil
    }
    ...
    func symlink<P: Pathish>(into dir: P) throws -> Path {
        ...
        case .directory?:
            ...
            try FileManager.default.createSymbolicLink(atPath: dst.path, withDestinationPath: path)
            return dst
        case .file?:
            throw CocoaError.error(.fileWriteFileExists)
        }
